...
`r Biocpkg("ImageArray")` can also read OME-TIFF files with all five dimensions (XYZCT)
available in the OME (Open Microscopy Environment) specifications. RBioFormats would drop
any of the dimensions with only one level. 
...
`r Biocpkg("ImageArray")` incorporates the `r Biocpkg("rome")` package to read 
OME-ZARR images.
...
library(rome)
x <- ome_read(path = file.path(td, "10501752.zarr"), lazy = TRUE)
as.ImageArray(x)
```

## SpatialData images and labels

`r Biocpkg("ImageArray")` can also use the `r Biocpkg("rome")` package to 
validate and represent image pyramids from the `r Biocpkg("SpatialData")` 
package

```{r sd}
if(!requireNamespace("SpatialData", quietly = TRUE))
  devtools::install_github("SpatialData")
if(!requireNamespace("SpatialData.validate", quietly = TRUE))
  devtools::install_github("SpatialData.validate")
path <- system.file("extdata", "blobs_v3.zarr", package="SpatialData")
x <- ome_read(file.path(path, "images/blobs_multiscale_image/"))
as.ImageArray(x)
```

```{r sd_labels}
x <- ome_read(file.path(path, "labels/blobs_multiscale_labels/"))
as.ImageArray(x)
```
