...
# create image array
output_h5 <- tempfile(fileext = ".h5")
imgarray2 <- writeImageArray(img, output = output_h5)
imgarray2
```
...
```{r translation}
imgarray_trans <- translate_transform(imgarray2,
                                      shift = c(100,20))
```
...
```{r scale}
imgarray_scale <- scale_transform(imgarray2, 
...
imgarray_affine <- affine_transform(imgarray2, 
                                    m = m, 
                                    filter = "bilinear")
```
...
imgarray_affine <- affine_transform(imgarray2, 
                                    m = m, 
                                    filter = "bilinear")
imgarray_scale <- scale_transform(imgarray_affine, 
                                  output.dim = c(200,300),
                                  filter = "bilinear")
imgarray_trans <- translate_transform(imgarray_scale,
                                      shift = c(100,20))
```
